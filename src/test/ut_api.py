...
class Ut_serverTestCase(unittest.TestCase):
    ...
    def test_001_set(self):
        r = requests.post('http://localhost:8000/bank/bank1/entry/entry1',
                          headers={"Content-Type": "text / plain"},
                          data="value 1")
        self.assertEqual(r.status_code, 200)

    def test_002_get(self):
        r = requests.get('http://localhost:8000/bank/bank1/entry/entry1')
        self.assertEqual(r.status_code, 200)
        r = r.json()
    ...
    def test_003_keys(self):
        ...
        r = requests.post('http://localhost:8000/bank/bank1/entry/entry2',
                          headers={"Content-Type": "text / plain"},
                          data="value 2")
        self.assertEqual(r.status_code, 200)

        r = requests.get('http://localhost:8000/bank/bank1/entries')
        self.assertEqual(r.status_code, 200)
        r = r.json()
        self.assertEqual(r["status"], "ok")
    ...
    def test_004_clear_bank(self):
        r = requests.put('http://localhost:8000/bank/bank1?operation=reset')
        self.assertEqual(r.status_code, 200)
    ...
    def test_005_banks(self):
        ...
        r = requests.post('http://localhost:8000/bank/bank2/entry/entry1',
                          headers={"Content-Type": "text / plain"},
                          data="value 1")
        self.assertEqual(r.status_code, 200)

        r = requests.get('http://localhost:8000/banks')
        self.assertEqual(r.status_code, 200)
        r = r.json()
    ...
    def test_005_error_get_unknownEntry(self):
        r = requests.get('http://localhost:8000/bank/bank1/entry/unknownEntry')
        self.assertEqual(r.status_code, 400)

    def test_006_error_get_unknownBank(self):
        r = requests.get('http://localhost:8000/bank/unknownBank/entry/unknownEntry')
        self.assertEqual(r.status_code, 400)

    def test_007_entries_unkwnowBank(self):
        r = requests.get('http://localhost:8000/bank/unknownBank/entries')
        self.assertEqual(r.status_code, 200)
